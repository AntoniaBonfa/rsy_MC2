...
create_cleveland_plot <- function(usermatrix = m_user_genre, moviematrix = df_movie_genre, cosim_matrix = m_neg_csim, userID, N){
  
  #Empfehlungen erstellen & mit Genres ergänzen
  df_top20 <- get_topn_recos(cosim_matrix, N)
  df_topN_movies <- rownames_to_column(moviematrix, "movie") %>% filter(movie %in% df_top20$movie)
  ...
  df_user_genre_cleveland <- rownames_to_column(as.data.frame((usermatrix)), "user")  %>%
    filter(user == userID) %>% 
    pivot_longer(!user, names_to = "genre", values_to = "n") %>% 
    mutate(tot = sum(n),
         pct = n / tot,
  ...
  labs(title = paste("Genre-Profil für Nutzer:in", userID),
...
```{r Clevelandplot für Nutzer}

for (user in users_neg) {
  clp <- create_cleveland_plot(userID = user,N = 20)
  print(clp)
...
for (Ns in Nlist) {
  clp <- create_cleveland_plot(userID = 133,N = Ns)
  print(clp)
}

for (Ns in Nlist) {
  clp <- create_cleveland_plot(userID = 555,N = Ns)
  print(clp)
}
