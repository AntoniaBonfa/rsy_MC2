...
dim1 = dim(m_neg_csim)[1]
dim2 = dim(m_neg_csim)[2]
    ...
    Top <- matrix_melt  %>% 
      ...
      mutate(rank = rep(1:nrow(),dim1)) %>%  # generate new column "rank", For each user, the rank will start from 1.
...
analyze_topn_recos <- function(df, users, N){
  for (user in users){
    recs <- get_topn_recos(df, user, N)
  }
