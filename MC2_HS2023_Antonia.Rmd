...
stat5point <- as.data.frame(t(quantile(m_csim_user_movie,na.rm = TRUE))) %>% 
  mutate(mean = mean(m_csim_user_movie, na.rm=TRUE), 
         NAs = sum(is.na(m_csim_user_movie)))
colnames(stats) <- c("Min", "first Qu", "Median", "third Qu", "Max", "Mean", "NAs")
stats

stat5point <- quantile(m_csim_user_movie, na.rm = TRUE)
means <- mean(m_csim_user_movie, na.rm = TRUE)
NAs <- sum(is.na(m_csim_user_movie))
