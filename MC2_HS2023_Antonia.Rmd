...
Wenn wir nun  wieder ein Histogram machen mit den Anzahl Likes pro Genre ist das Bild sehr ähnlich wie die Verteilung bei den Filmen. Das Bild mit den Genre-Kombinationen ist ebenfalls sehr ähnlich zum vorherigen mit den Filmen:
...
```
Wir haben 6 sehr unterschiedliche Dichte-Profile. User 241, 414 und 477 alle haben eher eine Häufung Nahe 0 (also viele sehr kleine Werte) und dann wieder Höhen in der Mitte - gegen das obere Ende nimmt es dann wieder ab. Dies deutet darauf hin, dass sie eher spezifische Genres mögen und nicht einfach alles ein bisschen - Filme, deren Genres also nicht im User-Profil vorkommen sorgen für tiefe Ähnlichkeiten. Die Position der Häufungen lassen auch Rückschlüsse auf den Genre-Geschmack der User zu: wenn die Häufing sehr tief ist, dann mag der User eher unbeliebte / unbekannte Genres (da dann wenige Filme das Genre haben). Ist der Spike eher hoch (so ab 0.6) hat die Person wohl eher ein Geschmack für die Hauptgenres Drama, Comedy und Action. 

Wenn wir uns noch User 640 anschauen scheint dieser eher ein Allrounder zu sein. Sehr wenig tiefe Werte aber sehr viele Werte nahe der Mitte deuten darauf hin, dass diese Person eigentlich fast alle Filmgenres mag. 

Bei User 710 & 526 scheint das ebenfalls der Fall zu sein: auch hier gibt es wenige Werte nahe 0. Der Unterschied zwischen User 526 und 710 scheint darin zu sein, dass User 526 vermutlich eher Comedy & Drama Filme mag (da wir ja bereits festgestellt haben, dass das die häufigsten Genres sind). 

Bei User 710  gibt es wenige Werte bei 0 aber die erste Häufung bei ca. 0.1 - diese Person hat also vermutlich fast alle Genres mal geliked, aber bei einige eher weniger. 
...
```
Sowohl im Histogram wie auch in den Boxplot sehen wir, dass mit zunehmendem N auch mehr tiefe Ähnlichkeiten dazu kommen. Das macht auch Sinn, da die Empfehlungen ja "schlechter" werden je tiefer man in der Rangliste absteigt. Ausserdem nimmt die Varianz zu; bei kleinem N ist die Ähnlichkeit der Top Empfehlungen sehr ähnlich zueinander. Dies kommt daher, da User in der Regel ja Filme mit ähnlichen Genres vorgeschlagen bekommen. Wenn der Genre-Mix identisch ist zwischen Filmen, dann ist deren Ähnlichkeit auch identisch. Bei zunehmendem N haben wir vermehrt Filme die vielleicht ein etwas anderen Genre-Mix enthalten und daher auch einen anderen Ähnlichkeits-Score haben. 
...
```
Wenn wir uns die Cleveland Plots der verschiedenen Nuzter:innen anschauen, so sehen wir, dass bei 
...
Persönlich google ich lieber zuerst selber, da der Code von ChatGPT gerade in R häufig nicht besonders gut ist und ich auch häufig nicht verstehe, was der Code jetzt genau macht. 
